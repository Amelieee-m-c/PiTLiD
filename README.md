...
| 指標 | 論文 | 舊版(論文文字猜的 CLR,單階段架構) | 新版 step-decay(官方 code) | 新版 CLR(官方 code + 真 CLR) | 新版 step-decay + **固定切分** |
|---|---|---|---|---|---|
| Accuracy | 99.45 ± 0.17% | 98.74 ± 0.47% | 98.67 ± 0.69% | 98.19 ± 1.00% | **99.20 ± 0.30%** |
| Precision | 98.84 ± 0.31% | 98.40 ± 0.92% | 98.08 ± 1.39% | 97.40 ± 1.62% | 98.81 ± 0.73% |
| Sensitivity / Recall | 99.10 ± 0.23% | 98.42 ± 0.63% | 98.64 ± 0.77% | 98.22 ± 0.92% | 99.12 ± 0.21% |
| F1 | 99.00 ± 0.23% | 98.38 ± 0.75% | 98.33 ± 1.10% | 97.77 ± 1.28% | 98.96 ± 0.46% |

意外的結果:**CLR 版本反而是四組裡最差的**,直接跟論文 Figure 5B「CLR 是
三種 LR 策略裡最好的」這個主張相反。原始四組全部卡在 98.2%-98.7% 之間,
標準差都明顯比論文的 0.17%-0.31% 大——這比較可能反映的是「30-shot 小樣本
本身訓練不穩定」這個共通問題,而不是 LR 策略選錯。`clr_step_size`(CLR
半週期長度)因為官方 `clr_callback.py` 遺失、一直是用猜的(2000),很可能是
CLR 版本表現不如預期的原因。

**2026-09-15 更新:「10 次獨立跑」改成固定切分後,大幅逼近論文。** 原本
每次跑都重新抽一次 30-shot 切分,把「抽到哪 30 張圖」的變異跟「訓練 seed」
的變異混在一起。改成只抽一次切分、10 次跑都共用(只變訓練 seed)後,
**標準差從 0.69% 壓到 0.30%(縮小超過 2 倍),平均值也從 98.67% 拉到
99.20%(跟論文的差距從 0.78pp 縮到 0.25pp)**——這是目前 5 個變體裡最接近
論文的一個,強烈支持論文的「10 次獨立跑」協定指的是固定一組切分、只變
訓練種子,而不是每次都重新抽切分。

**2026-09-17 更新:讀完整篇論文後測試「論文文字字面協定」,結果反而更差。**
論文 3.2.2/3.2.6/3.2.7/3.2.8 節描述的協定(單階段訓練、全部層從 epoch 0
就可訓練、50 epoch 全程 steps=200、真正有生效的 early stopping
patience=14、真的有用的 L2)跟官方碼目前 committed 的版本(兩階段、
early stopping 是死碼)不一樣。新增 `--single_stage`/`--early_stopping_patience`
測試這個字面讀法(固定切分 + `--single_stage --early_stopping_patience 14
--lr_strategy clr --weight_decay 1e-3`):

| 指標 | 論文 | 新版 step-decay + 固定切分 | 論文字面協定(單階段+真early stop+CLR+L2)+ 固定切分 |
|---|---|---|---|
| Accuracy | 99.45 ± 0.17% | **99.20 ± 0.30%** | 98.90 ± 0.16% |
| Precision | 98.84 ± 0.31% | 98.81 ± 0.73% | 98.64 ± 0.22% |
| Recall | 99.10 ± 0.23% | 99.12 ± 0.21% | 98.52 ± 0.25% |
| F1 | 99.00 ± 0.23% | 98.96 ± 0.46% | 98.57 ± 0.18% |

Early stopping 有正常觸發(10 個 seed 裡 8 個真的提早停在 epoch 29-48,
不是死碼、不是沒生效),標準差也確實壓得比 step-decay 版本更小(0.16%
vs 0.30%)——但收斂到的準確率反而更低,跟論文的差距從 0.25pp 拉大到
0.55pp。**結論:論文文字描述的協定沒有比官方碼實際的協定更好**,官方碼
(雖然 early stopping 是死碼)反而是目前證據上更接近論文真實產出檢查點
的版本。**目前沒有一個版本能宣稱完全「乾淨重現」論文的 99.45±0.17%**,
固定切分+官方碼 step-decay 版本(99.20±0.30%)仍然是目前 6 個變體裡最
佳逼近,不建議再繼續測試論文字面協定的其他排列組合。

另外也跑了論文的葡萄、桃子泛化資料集。舊版(單一 seed,Keras 版本,舊
架構):葡萄 99.09% accuracy、桃子 99.54% accuracy,都很接近論文近乎完美
的混淆矩陣。**2026-09-16 更新:改用官方架構 + 固定切分協定重跑成完整
10-run**(`src/run_multi_seed.py --crop {grape,peach}`,新增的 `--crop`
參數呼叫本來就存在但沒人接上去用的通用切分腳本
`data_prep/make_pitlid_split.py`):

| 資料集 | 舊版(單 seed,舊架構) | 新版(10-run,官方架構+固定切分) |
|---|---|---|
| 葡萄 4 類 | 99.09% | **99.29 ± 0.17%** |
| 桃子 2 類 | 99.54% | **99.71 ± 0.09%** |

論文目前沒找到針對葡萄/桃子的明確 mean±std 數字可以直接對照(跟蘋果的
99.45±0.17%不同,只查到「近乎完美的混淆矩陣」這種定性描述),所以這兩組
是紮實的新 10-run 結果,但還沒有確切的論文數字可以算 pp 差距。
...
- ~~10-run 協定每次都重新抽 30-shot 切分——要再確認論文是否其實是 10 次都
  用同一組切分。~~ **已測試(2026-09-15)**:固定切分版本標準差縮小超過
  2 倍、平均值更接近論文,是目前最佳逼近,但仍未完全乾淨重現 99.45±0.17%。
- ~~葡萄/桃子泛化資料集:已經實作並跑過(單一 seed,Keras 版本,舊架構),
  尚未用官方架構重跑。~~ **已完成(2026-09-16)**:兩者都已用官方架構 +
  固定切分跑完完整 10-run,結果見上方。論文目前沒找到針對這兩個資料集的
  明確 mean±std 可對照。
